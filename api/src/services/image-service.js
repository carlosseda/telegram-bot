...
module.exports = class ImageService {
  ...
  downloadImage = async (url) => {
    return new Promise((resolve, reject) => {
      const filename = path.join(__dirname, `../storage/shoots/snapshot_${Date.now()}.jpg`)
      ...
      https.get(url, response => {
        ...
        file.on('finish', () => {
          file.close()
          const storagePath = path.relative(__dirname, filename).replace(/\\/g, '/').replace('../', '')
          resolve({ success: true, filename, storagePath })
        })
      }).on('error', error => {
        fsSync.unlink(path.join(__dirname, `../storage/shoots/snapshot_${Date.now()}.jpg`))
        reject(error)
      })
    })
  }
