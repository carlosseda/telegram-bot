...
exports.create = async (req, res, next) => {
  try {
    let data = await Hero.create(req.body)
    data = data.toObject()
    data.id = data._id
...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const result = await Hero.find(whereStatement)
      .skip(offset)
      .limit(limit)
      .sort({ createdAt: -1 })
      .lean()
    ...
    const count = await Hero.countDocuments(whereStatement)
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await Hero.findById(id).lean().exec()
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await Hero.findByIdAndUpdate(id, req.body, { new: true }).lean().exec()
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await Hero.findByIdAndUpdate(id, { deletedAt: new Date() })
