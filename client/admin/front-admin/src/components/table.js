...
class Table extends HTMLElement {
  ...
  async render () {
    ...
    if (this.structure.tableButtons) {
      ...
      Object.keys(this.structure.tableButtons).forEach(tableButton => {
        ...
        if (this.structure.tableButtons[tableButton] === 'autocompleteButton') {
          tableButtonElement.classList.add('autocomplete-button')
          tableButtonElement.innerHTML = `
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.5 2a.5.5 0 0 1 .49.402c.137.683.404 1.266.814 1.708.406.437.977.763 1.778.897a.5.5 0 0 1 0 .986c-.8.134-1.372.46-1.778.897-.41.442-.677 1.025-.814 1.708a.5.5 0 0 1-.98 0c-.137-.683-.404-1.266-.814-1.708-.406-.437-.977-.763-1.778-.897a.5.5 0 0 1 0-.986c.8-.134 1.372-.46 1.778-.897.41-.442.677-1.025.814-1.708A.5.5 0 0 1 5.5 2Z"></path><path fill-rule="evenodd" d="M14.293 4.793a4.536 4.536 0 1 1 6.414 6.414l-7.612 7.612c-.479.479-.81.81-1.197 1.075a4.996 4.996 0 0 1-1.1.566c-.44.16-.902.238-1.57.349l-2.959.493-.029.005c-.16.027-.34.057-.496.069-.17.012-.44.017-.727-.106a1.5 1.5 0 0 1-.787-.787 1.525 1.525 0 0 1-.106-.727c.012-.156.042-.336.069-.497l.498-2.987c.111-.668.188-1.13.35-1.57a5 5 0 0 1 .565-1.1c.264-.387.596-.719 1.075-1.197l7.612-7.612Zm5 1.414a2.536 2.536 0 0 0-3.586 0l-.293.293L19 10.086l.293-.293c.99-.99.99-2.596 0-3.586Zm-11.136 7.55L14 7.914l3.586 3.586-5.843 5.843c-.564.564-.76.753-.973.9-.205.14-.427.253-.66.339-.243.089-.51.137-1.297.269l-2.596.432.432-2.596c.131-.787.18-1.055.269-1.297.085-.234.2-.455.34-.66.145-.214.335-.409.9-.973Z" clip-rule="evenodd"></path></svg>
          `
        }
  ...
  async getTableData () {
    ...
    this.data.forEach(element => {
      ...
      Object.keys(recordButtons).forEach((recordButton) => {
        ...
        if (recordButtons[recordButton] === 'load-data') {
          tableButton.classList.add('load-data-button')
          tableButton.dataset.id = element.id
          tableButton.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        if (recordButtons[recordButton] === 'process-data') {
          tableButton.classList.add('process-data-button')
          tableButton.dataset.id = element.id
          tableButton.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                          <path d="M6.5 20Q4.22 20 2.61 18.43 1 16.85 1 14.58 1 12.63 2.17 11.1 3.35 9.57 5.25 9.15 5.83 7.13 7.39 5.75 8.95 4.38 11 4.08V12.15L9.4 10.6L8 12L12 16L16 12L14.6 10.6L13 12.15V4.08Q15.58 4.43 17.29 6.39 19 8.35 19 11 20.73 11.2 21.86 12.5 23 13.78 23 15.5 23 17.38 21.69 18.69 20.38 20 18.5 20Z" />
        ...
        if (recordButtons[recordButton] === 'roi-selection') {
          tableButton.classList.add('roi-selection-button')
          tableButton.dataset.id = element.id
          tableButton.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        if (recordButtons[recordButton] === 'look-vision-source') {
          tableButton.classList.add('look-vision-source-button')
          tableButton.dataset.id = element.id
          tableButton.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
  ...
  async renderTableButtons () {
    ...
    tableSection.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('.load-data-button')) {
        const loadDataButton = event.target.closest('.load-data-button')
        let endpoint = import.meta.env.VITE_API_URL + this.getAttribute('endpoint') + '/load-dataset/' + loadDataButton.dataset.id
        ...
        try {
          ...
          if (response.status === 200) {
            document.dispatchEvent(new CustomEvent('message', {
              detail: {
                message: 'Subida de datos realizada correctamente',
                type: 'success'
              }
            }))
          }
          ...
          document.dispatchEvent(new CustomEvent('message', {
            detail: {
              message: 'Fallo al cargar los datos',
              type: 'error'
            }
          }))
        }
      ...
      if (event.target.closest('.look-vision-source-button')) {
        const lookVisionSourceButton = event.target.closest('.look-vision-source-button')
        let endpoint = import.meta.env.VITE_API_URL + this.getAttribute('endpoint') + '/look-vision-source/' + lookVisionSourceButton.dataset.id
      ...
      if (event.target.closest('.process-data-button')) {
        const processDataButton = event.target.closest('.process-data-button')
        let endpoint = import.meta.env.VITE_API_URL + this.getAttribute('endpoint') + '/process-data/' + processDataButton.dataset.id
        ...
        try {
          ...
          if (response.status === 200) {
            document.dispatchEvent(new CustomEvent('message', {
              detail: {
                message: 'Procesado de datos realizado correctamente',
                type: 'success'
              }
            }))
          }
          ...
          document.dispatchEvent(new CustomEvent('message', {
            detail: {
              message: 'Fallo al procesar los datos',
              type: 'error'
            }
          }))
        }
      ...
      if (event.target.closest('.roi-selection-button')) {
        const processDataButton = event.target.closest('.roi-selection-button')

        const data = {
          id: processDataButton.dataset.id,
          parent: this.parent,
          endpoint: this.getAttribute('endpoint')
        }

        document.dispatchEvent(new CustomEvent('openRoiSelection', {
          detail: data
        }))
      }
